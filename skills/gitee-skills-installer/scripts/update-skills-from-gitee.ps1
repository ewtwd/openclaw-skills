param(
  ...
  [string]$ZipUrl = "https://gitee.com/chen-zhuoxiao/openclaw-skills/repository/archive/master.zip",
  [switch]$DryRun,
  [switch]$KeepTempDir,
...
function Get-RepoUrlFromZipUrl {
  ...
  $match = [regex]::Match($Value, '^https://gitee\.com/([^/]+/[^/]+)/repository/archive/.+$')
  if (-not $match.Success) {
    throw "Cannot infer repo URL from ZipUrl: $Value"
  }

  return ("https://gitee.com/" + $match.Groups[1].Value + ".git")
}
...
if ($DryRun) {
  ...
  Write-Host "2. download Gitee zip"
